...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── 1. Create the Tremendous reward ───────────────────────
    ...
    const tremRes = await fetch(`${base}/orders`, {
      ...
      body: JSON.stringify({
        ...
        reward: {
          ...
          delivery: { method: "LINK" }, // we email our own branded link, not Tremendous's
    ...
    const tremendousId = trem?.order?.id ?? null;
    const rewardLink = reward?.delivery?.link ?? reward?.link ?? null;
...
function giftEmail(p: {
  ...
  <div style="max-width:480px;margin:0 auto;padding:32px 24px">
    <div style="font-weight:800;font-size:22px;color:#1C1A17">Givin<span style="color:#E9604C">.</span></div>
    <div style="background:#fff;border:1px solid #ECE7E0;border-radius:16px;padding:28px;margin-top:20px">
      ...
      <a href="${p.redeemUrl}" style="display:block;text-align:center;background:#E9604C;color:#fff;text-decoration:none;font-weight:700;font-size:16px;border-radius:12px;padding:14px;margin-top:20px">Redeem your gift</a>
